...
from app.orchestrator.workflow import run_full_workflow
from app.agents.agent_4_dispatcher import build_pdf_to_bytes, extract_candidate_contact
from app.database.relational import get_candidate_by_run_id

load_dotenv()

BASE_DIR = Path(__file__).resolve().parent.parent
# Only the DB directory is kept on disk — no upload or output folders
DB_DIR = BASE_DIR / "app" / "database_output"
...
class WorkflowRequest(BaseModel):
    ...
    send_email: bool = False
    to_email: Optional[str] = None
    # out_dir removed — no files are written to disk
...
class WorkflowResponse(BaseModel):
    ...
    company: Optional[str] = None
    role: Optional[str] = None
    pdf_download_url: Optional[str] = None  # /downloads/{run_id} — served from DB
    agent1_output: Dict[str, Any] = Field(default_factory=dict)
    agent2_output: Dict[str, Any] = Field(default_factory=dict)
    agent3_output: Dict[str, Any] = Field(default_factory=dict)
...
def save_upload_to_temp(file: UploadFile) -> tempfile.NamedTemporaryFile:
    """
    Save an uploaded file to a NamedTemporaryFile.
    The caller is responsible for closing it (which deletes the file on all platforms).
...
@asynccontextmanager
async def lifespan(app: FastAPI):
    # Only the DB directory needs to exist — no upload or output folders
    DB_DIR.mkdir(parents=True, exist_ok=True)
    yield
...
async def run_workflow_with_uploads(
    ...
    """
    Upload resume + JD, run the full workflow.
    Uploaded files are written to OS temp storage and deleted immediately after
    processing — nothing is stored permanently on disk.
    """
    tmp_resume = None
    tmp_jd = None

    try:
        tmp_resume = save_upload_to_temp(resume)
        tmp_jd = save_upload_to_temp(jd)
        ...
        # Delete temp files immediately after the workflow finishes
        for tmp in (tmp_resume, tmp_jd):
            if tmp is not None:
                try:
                    tmp.close()
                    Path(tmp.name).unlink(missing_ok=True)
...
async def run_workflow_from_paths(payload: WorkflowRequest) -> WorkflowResponse:
    """
    Run the workflow from pre-existing local file paths (useful for local testing).
    No files are written to disk by the workflow itself.
    """
    if not Path(payload.resume_path).exists():
        raise HTTPException(status_code=400, detail="resume_path does not exist")
...
@app.get("/downloads/{run_id}", tags=["Downloads"])
async def download_pdf(run_id: str):
    """
    Regenerate and stream the PDF for a past run directly from DB-stored agent outputs.
    No file is read from disk — the PDF is built in memory on every request.
    """
    record = get_candidate_by_run_id(run_id)
    if not record:
        raise HTTPException(status_code=404, detail=f"No run found for run_id={run_id!r}")

    agent3_data = record.get("agent_3_output")
    if not agent3_data:
        raise HTTPException(status_code=404, detail="Agent 3 output not found in database for this run.")
    ...
    filename = f"interview_qa_pack_{run_id}.pdf"
...
async def get_run_metadata(run_id: str):
    """
    Fetch metadata for a past run from the database.
    """
    record = get_candidate_by_run_id(run_id)
    if not record:
        raise HTTPException(status_code=404, detail=f"No run found for run_id={run_id!r}")
    ...
    return {
        ...
        "candidate_name": record.get("candidate_name"),
        "candidate_email": record.get("candidate_email"),
        "pdf_download_url": f"/downloads/{run_id}",
    }
